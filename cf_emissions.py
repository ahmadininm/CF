...
def main():
    ...
    # Sidebar for Save and Load Progress
    ...
    # ----------------------- Scenario Generation -----------------------
    st.subheader("Generate Scenario Suggestions (Optional)")
    ...
    if activities_description.strip() != "":
        ...
        st.info("Please describe your organization's activities to enable scenario generation.")
    
    # ----------------------- Scenario Planning -----------------------
    
    st.subheader("Scenario Planning (Editable Table)")
    
    # Let users define scenarios manually or use generated scenarios
    if activities_description.strip() != "":
    ...
    num_scenarios = st.number_input(
        ...
        min_value=1,
        step=1,
        value=max(1, st.session_state.edited_scenario_desc_df.shape[0] if not st.session_state.edited_scenario_desc_df.empty else 1),
        key="num_scenarios_input_2"
    )
    
    # Adjust the number of scenarios if changed
    current_scenarios = st.session_state.edited_scenario_desc_df.shape[0] if not st.session_state.edited_scenario_desc_df.empty else 0
    ...
    st.write("Please assign usage percentages to each scenario for each BAU item. Double-click a cell to edit.")
