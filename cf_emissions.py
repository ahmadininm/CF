...
def get_openai_version_importlib():
    """
    Retrieves the installed OpenAI package version using importlib.metadata.
    Returns:
        str: OpenAI package version or "Package not found."
    """
    try:
...
def save_session_state():
    ...
    state = {
        ...
        'edited_scenario_desc_df': st.session_state.get('edited_scenario_desc_df').to_json(),
...
def test_openai_linkage():
    """
    Tests the OpenAI API linkage by making a simple API call.
...
def main():
    ...
    # ----------------------- Sidebar: Save and Load Progress -----------------------
    ...
    st.sidebar.write("### Package Versions")
    ...
    try:
        ...
        st.sidebar.write("**Streamlit Version:** Not Found")
    ...
    with st.expander("🔍 Optional: Generate Scenarios Using AI"):
        st.write("You can optionally generate scenario suggestions based on your activities description using OpenAI's GPT.")
        if activities_description.strip() != "":
            ...
            st.info("Please provide an activities description to generate scenarios.")
    ...
    if activities_description.strip() != "" and not st.session_state.edited_scenario_desc_df.empty:
        ...
        num_scenarios = st.number_input(
            ...
            min_value=1,
            step=1,
            value=len(st.session_state.edited_scenario_desc_df),
            key="num_scenarios_input_2"
        )
    
        # Adjust the number of scenarios if changed
    ...
    # ----------------------- Additional Criteria -----------------------
    st.subheader("Additional Criteria for Scenario Evaluation")
