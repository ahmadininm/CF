...
- Percentages can exceed **100%** to indicate increased usage compared to BAU.
""")
            ...
            column_config={
                ...
                **{
                    f"{row['Scenario']} (%)": st.column_config.NumberColumn(
                        ...
                        min_value=0.0,
                        # Removed max_value=100.0 to allow percentages >100%
                        format="%.1f",
                        step=0.1
    ...
    for col in edited_scenario_df.columns[1:]:
        ...
        # Removed clipping to 100% to allow percentages >100%
        # edited_scenario_df[col] = edited_scenario_df[col].clip(lower=0.0, upper=100.0)

    # **Removed Percentage Sum Validation**
    # Previously, the sum of percentages per item was not allowed to exceed 100%.
    # Since you want to allow it, this validation has been removed.
