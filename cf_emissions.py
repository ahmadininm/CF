...
if selected_criteria:
    ...
    if st.button("Run the model"):
        ...
        for crit in selected_criteria:
            ...
            if all(values == 0):
                scaled_criteria_df[crit] = 0
            elif crit in ["Initial investment (£)", "Return on Investment (ROI)(years)"] or (other_name == crit and other_scale == "No"):
                scaled_criteria_df[crit] = 10 - 9 * (values - values.min()) / (values.max() - values.min()) if values.max() != values.min() else 0
            else:
                scaled_criteria_df[crit] = 1 + 9 * (values - values.min()) / (values.max() - values.min()) if values.max() != values.min() else 0
