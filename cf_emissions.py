...
def main():
    ...
    if missing_factors.any():
        ...
        # Optionally, prompt the user to input missing emission factors
        # For simplicity, fill missing values with 0.0 or a default value
    ...
    st.subheader("Optional: Generate Scenario Suggestions with AI")
    st.write("If you want, you can generate scenario suggestions based on your activities description using AI. This step is optional.")

    if activities_description.strip() != "":
        st.success("Activities description received.")
        st.write("Click the button below to generate scenario suggestions based on your description.")
        
        if st.button("Generate Scenarios with AI"):
        ...
        st.info("Please describe your organization's activities to optionally generate scenarios.")

    # ----------------------- Scenario Planning -----------------------
    st.subheader("Scenario Planning (Editable Table)")

    # Let users define scenarios manually or use generated ones
    if activities_description.strip() != "":
        if not st.session_state.edited_scenario_desc_df.empty:
            st.success("You have defined scenarios. You can now proceed to assign usage percentages.")
        else:
            st.info("You can define your scenarios manually or use the AI-generated suggestions above.")
    else:
    ...
    # Create or adjust the scenario descriptions DataFrame
    scenario_desc_columns = ["Scenario", "Description"]
    current_scenarios = st.session_state.edited_scenario_desc_df.shape[0]
    if int(num_scenarios) > current_scenarios:
        additional_scenarios = int(num_scenarios) - current_scenarios
        for i in range(additional_scenarios):
    ...
    st.write("Please assign usage percentages to each scenario for each BAU item. Ensure that the percentages are between 0 and 100.")
