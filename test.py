...
def load_session_state(uploaded_file):
    ...
    try:
        ...
        bau_data_loaded['Daily Cost (£/day)'] = pd.to_numeric(bau_data_loaded['Daily Cost (£/day)'], errors='coerce').fillna(0.0)
...
def get_emission_factor_from_openai(item_name, unit):
    ...
    """
    prompt = (
        f"Provide a best-guess numeric estimate of the emission factor for '{item_name}' in units of kg CO₂e per {unit}. "
...
def main():
    ...
    # Before running the main app, ask for user details
    if 'user_authenticated' not in st.session_state:
        st.session_state.user_authenticated = False

    if not st.session_state.user_authenticated:
        st.title("Welcome to the Sustainability Decision Assistant")
        st.write("Please provide the following information before proceeding:")
        ...
        st.write("- Your data will not be used for model training.")
        st.write("- Data may be used anonymously for academic research.")
        ...
        st.write(f"Hello {st.session_state.user_name} from {st.session_state.user_company}! Thank you for using our tool.")
        ...
        if 'bau_data' not in st.session_state:
            ...
            st.session_state.bau_data = pd.DataFrame({
                ...
                "Daily Cost (£/day)": [0.0] * len(default_items),
                "Unit": [i.split("(")[-1].replace(")","") for i in default_items]
            })
        ...
        for i in range(len(bau_data)):
            ...
            bau_data.loc[i, "Daily Cost (£/day)"] = st.number_input(
                ...
                min_value=0.0,
                step=0.1,
                value=bau_data.loc[i, "Daily Cost (£/day)"],
                key=f"bau_cost_{i}"
            )
        ...
        if add_custom:
            ...
            for i in range(num_custom_items):
                ...
                c_cost = st.number_input(
                    f"Custom Item {i + 1} Daily Cost (£/day):",
                    min_value=0.0,
                    step=0.1,
                    value=0.0,
        ...
        st.subheader("Operational Cost Savings Compared to BAU (%)")
        st.bar_chart(results_df.set_index("Scenario")["Cost Saving (%)"], use_container_width=True)
        ...
        criteria_options = {
            ...
            "Initial investment (£)": "Enter cost. Will be normalized (lower cost = better).",
            "Return on Investment (ROI)(years)": "Enter ROI years. Will be normalized (lower = better).",
        ...
        if selected_criteria and 'edited_criteria_df' in st.session_state and not st.session_state.edited_criteria_df.empty:
            if st.button("Run Model"):
                if st.session_state.edited_criteria_df.isnull().values.any():
                    ...
                    Email: {st.session_state.user_email if st.session_state.user_email else "Not provided"}
                    
                    Top Scenario: {top_scenario[0] if len(top_scenario)>0 else "N/A"}
                    BAU Annual Emissions: {total_annual_bau:.2f} kg CO₂e/year
                    BAU Annual Cost: £{total_annual_cost:.2f}/year
                    """
